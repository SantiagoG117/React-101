...
const usePostsByUser = (userId: number | undefined) => {
  ...
  return useQuery<Post[], Error>({
    /* 
      When dealing with hierarchical data, we must structure the query Key following a hierarchical
      strucuture that represents the relationship between the data. In this case user -> posts.

      The pattern is the same pattern that we use when designing URLs for APIs: 
      user/userID/posts
